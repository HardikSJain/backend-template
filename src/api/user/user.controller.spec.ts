...
describe('UserController', () => {
  ...
  // Unit tests for getCurrentUser method can be added separately
  ...
  describe('createUser', () => {
    ...
    describe('CreateUserReqDto', () => {
      ...
      it('should fail with empty phone', async () => {
        createUserReqDto.phone = '' as unknown as string;
        const errors = await validate(createUserReqDto);
        expect(errors.length).toBeGreaterThan(0);
      });

      // password validations removed in OTP-only flow
  ...
  // Unit tests for findAllUsers/loadMoreUsers can be added separately
  ...
  // Unit tests for update/remove/changePassword can be added separately
});
