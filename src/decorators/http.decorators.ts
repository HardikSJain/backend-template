...
// Use primitive types directly to satisfy lint rules
...
export const ApiAuth = (options: IApiAuthOptions = {}): MethodDecorator => {
  ...
  let okDecorator: MethodDecorator;
  if (isPaginated) {
    okDecorator = ApiPaginatedResponse(ok);
  } else if (options.statusCode === 201) {
    okDecorator = ApiCreatedResponse(ok);
  } else {
    okDecorator = ApiOkResponse(ok);
  }
